...
public:
    ...
    ~DateLeafReader();
    async_simple::coro::Lazy<Result<SegmentPostingsVec>> Lookup(uint64_t leftTerm, uint64_t rightTerm,
                                                               docid64_t baseDocId, autil::mem_pool::Pool* sessionPool,
                                                               file_system::ReadOption option,
                                                               InvertedIndexSearchTracer* tracer) noexcept;

private:
    async_simple::coro::Lazy<ErrorCode> FillSegmentPostings(const DateTerm::Ranges& ranges, docid64_t baseDocId,
                                                           const std::shared_ptr<SegmentPostings>& dateSegmentPostings,
                                                           autil::mem_pool::Pool* sessionPool,
                                                           file_system::ReadOption option,
                                                           InvertedIndexSearchTracer* tracer) noexcept;
    async_simple::coro::Lazy<Result<SegmentPosting>> FillOneSegment(dictvalue_t value, docid64_t baseDocId,
                                                                   autil::mem_pool::Pool* sessionPool,
                                                                   file_system::ReadOption option,
                                                                   InvertedIndexSearchTracer* tracer) noexcept;
