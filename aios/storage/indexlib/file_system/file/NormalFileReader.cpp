...
async_simple::Future<FSResult<size_t>> NormalFileReader::ReadAsync(void* buffer, size_t length, size_t offset,
...
async_simple::Future<FSResult<size_t>> NormalFileReader::PrefetchAsync(size_t length, size_t offset,
                                                                      ReadOption option) noexcept
{
    return _fileNode->PrefetchAsync(length, offset, option);
}

async_simple::Future<FSResult<uint32_t>> NormalFileReader::ReadVUInt32Async(size_t offset, ReadOption option) noexcept
{
