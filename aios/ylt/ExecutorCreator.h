...
public:
    ...
    virtual std::unique_ptr<async_simple::Executor> Create(const Parameters &param) = 0;
...
public:
    static std::unique_ptr<async_simple::Executor> Create(const std::string &type, const Parameters &parameters) {
        auto &inst = Instance();
...
private:
    ...
    std::unique_ptr<async_simple::Executor> DoCreate(const std::string &type, const Parameters &parameters);
